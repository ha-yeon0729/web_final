...
<html>
    ...
    <body>
        ...
        <hr width="100%" color ="sky blue" size="2">
        <h1><?=$article['title']?></h1>

        <?=$article['sentence']?><br><br>


        <?php echo '<img src='.$show['file_address'].'>';?><br><br>

        <?= "파일명 : "?><?= $article['file_name'];?>
        <form action="download.php" method="POST">
            <input type="submit" name="download_button" value= "파일 다운로드">
            <input type="hidden" name="addr" id="addr" value=<?=$article['file_name']?>>
        </form>
